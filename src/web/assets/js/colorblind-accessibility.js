// Colorblind Accessibility Module
class ColorblindAccessibility {
  constructor() {
    ...
    // Colorblind-friendly palette using blue-orange contrast
    this.colorblindPalette = [
      ...
      ">
        ...
        ">
          <div class="toggle-switch" style="
            ...
            ${this.isColorblindMode ? 'background: #3b82f6;' : ''}
          ">
            <div class="toggle-slider" style="
              position: absolute;
              top: 2px;
              left: ${this.isColorblindMode ? '22px' : '2px'};
              width: 20px;
              height: 20px;
              background: white;
              border-radius: 50%;
  ...
  updateMapColors() {
    const palette = this.isColorblindMode ? this.colorblindPalette : this.standardPalette;
  ...
  updateLegend() {
    ...
    if (legend && window.updateLegendColors) {
      window.updateLegendColors(legend, this.isColorblindMode);
    }
  }
  ...
  showModeChangeNotification() {
    ...
    const modeText = this.isColorblindMode ? 'Colorblind Friendly' : 'Standard';
    ...
    notification.innerHTML = `
      <div style="
        ...
        background: ${this.isColorblindMode ? '#dbeafe' : '#dcfce7'};
        color: ${this.isColorblindMode ? '#1e40af' : '#166534'};
        padding: 12px 20px;
        border-radius: 8px;
        border: 1px solid ${this.isColorblindMode ? '#bfdbfe' : '#bbf7d0'};
      ...
      ">
        <span style="margin-right: 8px;">${this.isColorblindMode ? '🔵' : '🟢'}</span>
  ...
  getCurrentPalette() {
    return this.isColorblindMode ? this.colorblindPalette : this.standardPalette;
  }

  // Method to check if colorblind mode is active
